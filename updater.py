...
INDEX = {"name":"中证红利低波50","code":"H30269","ticker":"H30269.SS"}
...
ALL_TICKERS = [INDEX["ticker"]] + [x["ticker"] for x in STOCKS]
...
def ticker_frame(df, ticker):
    ...
    if isinstance(df.columns, pd.MultiIndex):
        # group_by=ticker => ticker at first level
        try:
            x=df[ticker].copy()
        except Exception:
            return pd.DataFrame()
...
def build_daily_cache():
    ...
    cache = {
        "_source":"Yahoo Finance via yfinance",
...
def download_intraday():
    # 5m bars: used only to get the newest available price, in one bulk request
    ...
    try:
        ...
        print("Intraday bulk download failed:",repr(e))
        return pd.DataFrame()
...
def latest_from_intraday(df,ticker):
    ...
    if x.empty:
        return None,None
    p=num(x.iloc[-1]["Close"])
    ts=x.index[-1]
    return p, str(ts)

def calc(cache_item, current):
    c=[num(x) for x in cache_item["closes"]]
    h=[num(x) for x in cache_item["highs"]]
    c=[x for x in c if x is not None]
    h=[x for x in h if x is not None]
    if len(c)<250:
        raise ValueError("history < 250")
    base=c[-249:]
    ma250=float(np.mean(base+[current]))
...
def main():
    ...
    # Build once if missing. Rebuild daily only after 16:10 Beijing time.
    need_build = INDEX["ticker"] not in cache or sum(t in cache for t in ALL_TICKERS) < 45
    if now.hour>=16 and cache.get("_history_date") != now.strftime("%Y-%m-%d"):
        need_build=True
    ...
    def produce(item):
        ...
        if p is None:
            # Fallback to last valid daily close: action still succeeds instead of crashing
            p=num(h["closes"][-1])
            ts=h["dates"][-1]+" close"
        if not p:
            return None
        m=calc(h,p)
        m["market_date"]=h["dates"][-1]
        m["quote_time"]=ts
        return m

    idx=produce(INDEX)
    if idx is None:
        raise RuntimeError("No H30269 data from Yahoo cache/intraday")
    ...
    for s in STOCKS:
        try:
            m=produce(s)
            if m:
                stock_rows.append({"name":s["name"],"code":s["code"],"ok":True,**m})
                ok+=1
            else:
    ...
    payload={
        ...
        "market_date":idx.get("market_date","--"),
        "index":idx,
        "stocks":stock_rows,
        "constituent_count":len(STOCKS),
        "valid_stock_count":ok
    }
    OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8")
    print("SUCCESS",payload["updated_at"],"valid stocks:",ok)
